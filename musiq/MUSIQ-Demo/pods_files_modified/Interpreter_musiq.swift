...
public final class Interpreter {
    ...
    public func input(at index: Int) throws -> Tensor {
        ...
        let byteCount = 602112//TfLiteTensorByteSize(cTensor)
    ...
    public func input(data: Data, at index: Int) throws -> Tensor {
        let maxIndex = inputTensorCount - 1
        guard case 0...maxIndex = index else {
            throw InterpreterError.invalidTensorIndex(index: index, maxIndex: maxIndex)
        }
...
//        let data = Data(bytes: bytes, count: byteCount)
    ...
    @discardableResult
    public func copy(_ data: String, toInputAt index: Int) throws -> Tensor {
        let maxIndex = inputTensorCount - 1
        guard case 0...maxIndex = index else {
            throw InterpreterError.invalidTensorIndex(index: index, maxIndex: maxIndex)
        }
        ...
        do {
            try self.resizeInput(at: 0, to: Tensor.Shape([1, 224, 224, 3]))
//            try self.resizeInput(at: 0, to: Tensor.Shape())
        ///     Note: After resizing an input tensor, the client **must** explicitly call
        ///     `allocateTensors()` before attempting to access the resized tensor data or invoking the
        ///     interpreter to perform inference.
            try allocateTensors()
        ...
        guard let cTensor: UnsafeMutablePointer<TfLiteTensor> = TfLiteInterpreterGetInputTensor(cInterpreter, Int32(index)) else {
            throw InterpreterError.allocateTensorsRequired
        }
        
        let byteCount = TfLiteTensorByteSize(cTensor)
        print("TensorByteSize:\(byteCount)")
...
//        }
        ...
        // Create an UnsafeMutableBufferPointer for the input tensor
//        guard let inputBuffer = cTensor.pointee.data.data
//            .assumingMemoryBound(to: Float.self)
//            .bindMemory(to: Float.self, capacity: cTensor.shape.dimensions.reduce(1, *))
//            else {
//                fatalError("Failed to create input buffer.")
//        }
        
//        let typeName = TfLiteTypeGetName(cTensor.pointee.type)
//        print("TfLiteTypeName:\(typeName)")
        
//        let inputBytes = Array(string.utf8)
//        cTensor.copyString(inputBytes, toByteOffset: 0, replacing: true)
//        inputBuffer.copyBytes(from: inputBytes, count: inputBytes.count)
        
        var inputData = data.data(using: .utf8)
        print("inputData:\(inputData)")
        
        let inputBytes = data.utf8CString.map { UInt8($0) }
        print("inputBytes:\(inputBytes)")
        
//        let inputTensorData = cTensor.pointee.data.data
//        let floats = data.toArray(type: Float32.self)
        
//        let status = inputBytes.withUnsafeBytes {
//            TfLiteTensorCopyFromBuffer(cTensor, $0.baseAddress, inputBytes.count)
//        }
        
//        let status = TfLiteTensorCopyFromBuffer(cTensor, &inputData, inputBytes.count)
        // REQUIRES: input_data_size == TfLiteTensorByteSize(tensor)
        let status = TfLiteTensorCopyFromBuffer(cTensor, &inputData, 0)
...
//        return try input(at: index)
        return try input(data: inputData!, at: index)//test
    }
